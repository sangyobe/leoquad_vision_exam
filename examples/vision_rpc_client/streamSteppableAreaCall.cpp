...
StreamSteppableAreaCall::StreamSteppableAreaCall(ServiceType::Stub *stub, grpc::CompletionQueue *cq, void *udata)
...
StreamSteppableAreaCall::~StreamSteppableAreaCall() {
    // LOG(info) << "StreamSteppableAreaCall[" << _id << "] Delete call."; // Do not output log
    ...
    if (this->_call_state == CallState::WAIT_FINISH) {
        ...
        if (this->_call_state == CallState::WAIT_CONNECT) {
            LOG(info) << "StreamSteppableAreaCall[" << _id << "] Start call.";
            {
                ...
                _request.mutable_area()->set_unsteppables_count(100 - (_steppable_count % 100));
                _steppable_count++;
            ...
            LOG(info) << "StreamSteppableAreaCall[" << _id << "] Write done! Write another...(" << _steppable_count << ")";
            {
                std::lock_guard<std::mutex> lock(this->_proc_mtx);
            ...
            LOG(err) << "StreamSteppableAreaCall[" << _id << "] Invalid call state (" << static_cast<int>(_call_state) << ")";
            return false;
        }
    }
    else {
